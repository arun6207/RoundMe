...
extension UsersViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.userNameLabel.text = users[indexPath.row].results.first?.fullname
        cell.userEmailLabel.text = users[indexPath.row].results.first?.email
        if let userImage  = users[indexPath.row].results.first?.userPic{
             cell.userImageView.downloadImageFrom(link:userImage, contentMode: .scaleAspectFit)
        }
        return cell
    }
}
